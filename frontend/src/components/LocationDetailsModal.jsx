import { CheckCircle2, Edit3, MapPin, X, XCircle } from "lucide-react";
...
const LocationDetailsModal = ({ location, isOpen, canManage, onClose, onEdit, onToggleStatus }) => {
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">Inventory location details</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto px-6 py-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-800">
                  {active ? "Location is active" : "Location is inactive"}
                </p>
                <p className="mt-0.5 text-xs text-slate-500">
                  {active
                    ? "This location is available for inventory operations."
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Location ID</p>
              <p className="mt-1.5 text-sm font-semibold text-slate-800">#{location.id}</p>
            </div>

            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Created</p>
              <p className="mt-1.5 text-sm font-semibold text-slate-800">{formatDate(location.created_at)}</p>
            </div>

            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 sm:col-span-2">
              <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Last Updated</p>
              <p className="mt-1.5 text-sm font-semibold text-slate-800">{formatDate(location.updated_at)}</p>
            </div>
          </div>
          ...
          <div className="mt-5">
            ...
            <div className="mt-2 rounded-xl border border-slate-100 bg-slate-50 px-4 py-4">
